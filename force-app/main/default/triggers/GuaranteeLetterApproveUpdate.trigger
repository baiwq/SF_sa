trigger GuaranteeLetterApproveUpdate on guaranteeLetter__c (before update) {
if(trigger.new.size()>100){
      return;
   }else{
      for(guaranteeLetter__c newCC:trigger.new){
              guaranteeLetter__c  oldCC = Trigger.oldMap.get(newCC.id);
