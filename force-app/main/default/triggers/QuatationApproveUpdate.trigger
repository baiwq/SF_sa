trigger QuatationApproveUpdate  on quatation__c (before update) {
if(trigger.new.size()>100){
      return;
   }else{
          for(quatation__c newCC:trigger.new){
            quatation__c  oldCC = Trigger.oldMap.get(newCC.id);
             
            newCC.OwnerManager__c = newCC.OwnerManagerid__c;

            ImpFindApprover  IFA = new ImpFindApprover();

               if(newCC.MainProductGroup__c != null){
                   if(newCC.MainProductGroup__c == '输变电业务单元-电网保护自动化事业部' && newCC.projectBigAmountCost__c > 0 ){
                   ...
                   }else if(newCC.MainProductGroup__c == '发电及用电业务单元-电站保护自动化解决方案' || newCC.MainProductGroup__c == '发电及用电业务单元-企业电力保护自动化解决方案' || newCC.MainProductGroup__c == '发电及用电业务单元-仿真事业部' || newCC.MainProductGroup__c == '发电及用电业务单元-自控解决方案' || newCC.MainProductGroup__c == '发电及用电业务单元-工业物联网解决方案' || newCC.MainProductGroup__c == '发电及用电业务单元-安全用电解决方案'){
                       if((newCC.bidPriceProjectMarginRateFul__c > 28&& newCC.biddingTotalPriceFormula__c < 2000000) || (newCC.bidPriceProjectMarginRateFul__c < 28&& newCC.biddingTotalPriceFormula__c < 500000) ){
                            ...
                            newCC.cpywdyfzr__c = '00528000005AScp';
                       }
               	   }else{
                       String LeadingCadrelike = newCC.MainProductGroup__c;
                       List<StaffingTables__c> Staffing_list = [select LeadingCadre__c from StaffingTables__c where name =:LeadingCadrelike ];
                       String LeadingCadre = '';
