//TestApprovalCPQ
trigger ContractChangeApprovalCPQ on Contractchange__c (before update) {
    for(Contractchange__c cc:Trigger.new){
        Contractchange__c oldcc = Trigger.oldMap.get(cc.id);
        if(!System.Test.isRunningTest()){
          if(cc.StartId__c == '是'){
          //if(cc.isfindSupply__c == '是'){
                if((oldcc.approvalStatus__c =='草稿' && cc.approvalStatus__c == '提交待审批')||(oldcc.approvalStatus__c =='审批拒绝' && cc.approvalStatus__c == '提交待审批')){
                    if(cc.updateghfw__c == null && cc.sfscghfw__c != '是'){
                        cc.addError('还未上传供货范围清单，不能提交审批！');
                    }else{
                        ContractChangeCPQ.getSubmitinfo(cc.id, cc.ContractCode__c, cc.Name,cc.NewprojectType__c);
                    }
                }
                String contractAmount = cc.contractAmount__c +'';
                if(cc.recordType__c == '合同金额变更'){
                    contractAmount = cc.contractAmountAfterChange__c +'';
                }
                if(cc.sfscghfw__c == '是'){
                    if(oldcc.approvalStatus__c =='提交待审批' && cc.approvalStatus__c == '审批通过'){
                        ContractChangeCPQ.getApproval(cc.id, cc.ContractCode__c, cc.Name,'3',cc.NewprojectType__c,contractAmount);
                    }
                    
                    if(oldcc.approvalStatus__c =='提交待审批'&& cc.approvalStatus__c == '审批拒绝'){
                        ContractChangeCPQ.getApproval(cc.id, cc.ContractCode__c, cc.Name,'2',cc.NewprojectType__c,contractAmount);
                    }
