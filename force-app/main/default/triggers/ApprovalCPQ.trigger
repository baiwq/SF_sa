//TestApprovalCPQ
trigger ApprovalCPQ on contractreview__c (before update) {
    for(contractreview__c cc:Trigger.new){
        contractreview__c oldcc = Trigger.oldMap.get(cc.id);        
        if(cc.StartId__c == '是'){
          String opportunityCode = cc.opportunityCode__c;
           if(cc.TBOpportunity__c != null ){
               opportunityCode =  cc.ParentOpportunity__c ;
            }  
            
          if (!System.Test.isRunningTest()) {
              
              if(cc.sfscghfw__c != '是'){}else{
                  
                  
                  //调用接口
                  if(cc.contractAmount__c != oldCC.contractAmount__c ){
                      ContractreviewCPQ.getRate(cc.id, cc.Name, cc.contractAmount__c+'');
                  }

                  
                  if((oldcc.approvalStatus__c =='草稿' || oldcc.approvalStatus__c =='提交待审批') && cc.approvalStatus__c == '审批通过'){

                    if(cc.NewprojectType__c == '2'){
                      ContractreviewCPQ.getApproval(cc.id, null , null, cc.Name,'3',cc.NewprojectType__c,cc.contractAmount__c+'');
                    }else{
                      ContractreviewCPQ.getApproval(cc.id, opportunityCode , cc.quatationName__c, cc.Name,'3',cc.NewprojectType__c,cc.contractAmount__c+'');
                  ...
                  if((oldcc.approvalStatus__c =='草稿' ||oldcc.approvalStatus__c =='提交待审批')&& cc.approvalStatus__c == '审批拒绝'){
                    ...
                    if(cc.NewprojectType__c == '2'){
                      ContractreviewCPQ.getApproval(cc.id, null, null, cc.Name,'2',cc.NewprojectType__c,cc.contractAmount__c+'');
                    }else{
                      ContractreviewCPQ.getApproval(cc.id, opportunityCode , cc.quatationName__c, cc.Name,'2',cc.NewprojectType__c,cc.contractAmount__c+'');
